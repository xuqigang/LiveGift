...
//        //获取userId中的数字
//        NSScanner *scanner = [NSScanner scannerWithString:userID];
...
//        if ([userID integerValue] % 2) {
        if (self.queue1.operations.count < self.queue2.operations.count) {
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, 10, self.parentView.frame.size.width / 2+30, UIScale(55));
            ...
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, self.parentView.frame.size.height -10 - UIScale(55), self.parentView.frame.size.width / 2 + 30, UIScale(55));
                op.presentView.originFrame = op.presentView.frame;
